...
class LoewnerMorphology::Conversions {
	...
	public:
		...
		/*
		 * Converts an immage array of values of type T to the image array of double values. It is assumed that conversion is legal.
		 * Memory for both arrays must be allocated before entering the method.
		 */
		template<typename T>
		static void type2double(const T *imgType, double *imgDouble, int size);

		/*
		 * Converts an image array of double values to the image array of values of type T. It is assumed that conversion is legal.
		 * Memory for both arrays should be allocated before entering the method.
		 */
		template<typename T>
...
void LoewnerMorphology::Conversions::mhcl2rgb(const T *h, const T *c, const T *l, T *r, T *g, T *b, int size) {
	#pragma omp parallel for
		...
		T l_val = (l[i] + 1) / 2;
		T s_val = (delta < 1e-4) ? 0 : delta / (1 - std::abs(l[i]));
		
		T q = (l_val < 0.5) ? (l_val * (1 + s_val)) : (l_val + s_val - (l_val * s_val));
		T p = 2 * l_val - q;

		r[i] = std::round(hueToRgb(p, q, h[i] + (T) 1 / 3) * RGB_MAX);		
		g[i] = std::round(hueToRgb(p, q, h[i]) * RGB_MAX);		
		b[i] = std::round(hueToRgb(p, q, h[i] - (T) 1 / 3) * RGB_MAX);			
	}
}
...
void LoewnerMorphology::Conversions::mhcl2matrix(const T *h, const T *c, const T *l, LoewnerMorphology::MorphColorMatrix *vector, int size) {
	...
	for (int i = 0; i < size; i++) {
		...
		T value = 2 * PI * hv;
		T x = cv * std::cos(value);
		T y = cv * std::sin(value);  
