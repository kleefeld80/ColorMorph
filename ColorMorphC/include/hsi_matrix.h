...
/*
 * Represents a HSI matrix representing HSI image values. H stands for hue, S stands for saturation and I stands for intensity.
 * HSIMatrix is a 2x2 symetric matrix with the hue value as a top-left element, saturation values as both antidiagonal elements and
 * the intensity value as the a bottom-right element.
 */
template<typename T>
struct HSIMatrix {
        T h;
...
void __host__ __device__ HSIMatrix<T>::rgb2HSIMatrixVector(S *r, S *g, S *b, HSIMatrix<T> *vector, int size) {
        ...
        for (int i = 0; i < size; i++) {
                ...
                T den = std::sqrt((r_val - g_val) * (r_val - g_val) + (r_val - b_val) * (g_val - b_val));
